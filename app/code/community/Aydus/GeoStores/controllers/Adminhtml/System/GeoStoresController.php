...
class Aydus_GeoStores_Adminhtml_System_GeoStoresController extends Mage_Adminhtml_Controller_Action
{
	...
	public function updateGeoipAction()
	{
		...
		$this->_redirect('adminhtml/system_config/edit', array('section'=>'aydus_geostores'));		
	}
    ...
    public function selectAction()
    {
    	...
    	if ($store && $selectType && $val){
    		...
    		switch ($selectType){
    			...
    			case 'op' :
    				$result['error'] = false;
    				$allowedCountriesOnly = (in_array($op, array('!=','!()'))) ? true : false;
    	...
    	if ($post = $this->getRequest()->getPost()){
    		...
    		try {
    			...
    			if (is_array($stores) && count($stores)>0){
    				...
    				$rules = Mage::getModel('aydus_geostores/rule')->getCollection();
                    ...
                    foreach ($stores as $i=>$store){
                    	...
                    	$rule = Mage::getModel('aydus_geostores/rule');
                    ...
                    $this->_getSession()->addSuccess(Mage::helper('aydus_geostores')->__('Rules have been saved.'));
    			...
    			$this->_getSession()->addException($e,
    					Mage::helper('aydus_geostores')->__('An error occurred while saving the rules.'));
    		}
