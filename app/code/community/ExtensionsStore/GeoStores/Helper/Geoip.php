 ...
 */
class ExtensionsStore_GeoStores_Helper_Geoip extends Mage_Core_Helper_Abstract {
	protected $_geoip;
	...
	public function __construct() {
		try {
			...
			$libPath = Mage::getBaseDir () . DS . 'lib' . DS . 'ExtensionsStore' . DS . 'geoipcity.inc';
			...
			$geoipDb = Mage::getStoreConfig ( 'extensions_store_geostores/configuration/geoip_db' );
			
			if ($geoipDb && file_exists ( Mage::getBaseDir () . $geoipDb )) {
				
				$geoipDb = str_replace ( '/', DS, Mage::getBaseDir () . $geoipDb );
				$path = $geoipDb;
			} else if (file_exists ( Mage::getBaseDir ( 'var' ) . DS . 'extensions_store' . DS . 'geostores' . DS . 'GeoIPCity.dat' )) {
				
				$path = Mage::getBaseDir ( 'var' ) . DS . 'extensions_store' . DS . 'geostores' . DS . 'GeoIPCity.dat';
			} else {
			...
			Mage::log ( $e->getMessage (), null, 'extensions_store_geostores.log' );
		}
	}
	...
	public function updateGeoip() {
		...
		try {
			...
			$url = Mage::getStoreConfig ( 'extensions_store_geostores/configuration/geoip_download_url' );
			...
			if ($url) {
				...
				if (! curl_errno ( $ch )) {
					...
					if (preg_match ( '/Content-Disposition: .*filename=([^\s]+)/', $headers, $matches )) {
						...
						$varPath = Mage::getBaseDir ( 'var' ) . DS . 'extensions_store';
						...
						if (file_exists ( $dest )) {
							...
							$result ['error'] = false;
							$result ['data'] = Mage::helper ( 'extensions_store_geostores' )->__ ( 'GeoIP Database has been updated' );
						} else {
							
							$result ['error'] = true;
							$result ['data'] = Mage::helper ( 'extensions_store_geostores' )->__ ( 'An error occurred while moving the database' );
						}
					} else {
						
						$result ['error'] = true;
						$result ['data'] = Mage::helper ( 'extensions_store_geostores' )->__ ( 'An error occurred during download' );
					}
				} else {
					
					$result ['error'] = true;
					$result ['data'] = Mage::helper ( 'extensions_store_geostores' )->__ ( 'An error occurred executing the download' );
				}
				...
				$result ['error'] = true;
				$result ['data'] = Mage::helper ( 'extensions_store_geostores' )->__ ( 'Please set the download url' );
			}
		} catch ( Exception $e ) {
			
			Mage::log ( $e->getMessage (), null, 'extensions_store_storelegal.log' );
			$result ['error'] = true;
