 ...
 */
class ExtensionsStore_GeoStores_Model_Rule extends Mage_Core_Model_Abstract {
	/**
	 * Collection for singleton
	 *
	 * @var ExtensionsStore_GeoStores_Model_Resource_Rule_Collection
	 */
	...
	public function getRulesCollection() {
		if (! $this->_collection) {
			...
			$collection->getSelect ()->joinLeft ( array (
					'region_table' => $collection->getMainTable () 
			), 'main_table.rule_id = region_table.parent_id', array (
					'region_table.value AS region' 
			) );
			$collection->getSelect ()->joinLeft ( array (
					'city_table' => $collection->getMainTable () 
			), 'region_table.rule_id = city_table.parent_id', array (
					'city_table.value AS city' 
			) );
	...
	public function populate($normalizedData) {
		...
		if ($city) {
			...
			$countryRule = Mage::getModel ( 'extensions_store_geostores/rule' );
			$countryData = array (
					'store' => $store,
					'key' => 'country',
					'op' => '==',
			...
			$regionRule = Mage::getModel ( 'extensions_store_geostores/rule' );
			$regionData = array (
					'store' => $store,
					'parent_id' => $countryId,
					'key' => 'region',
